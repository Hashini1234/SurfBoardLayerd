...
public class GuideDaoImpl implements GuideDao {
    ...
    public GuideEntity getById(String id) throws SQLException, ClassNotFoundException {
        ResultSet rs = SQLUtil.executeQuery("SELECT * FROM Guide WHERE guide_id=?", id);
        if (rs.next()) {
    ...
    public boolean delete(String id) throws SQLException, ClassNotFoundException {
        return SQLUtil.executeUpdate("DELETE FROM Guide WHERE guide_id=?", id);
    }

    @Override
    public String generateNewId() throws SQLException, ClassNotFoundException {
        ResultSet rs = SQLUtil.executeQuery("SELECT guide_id FROM Guide ORDER BY guide_id DESC LIMIT 1");
        if (rs.next()) {
